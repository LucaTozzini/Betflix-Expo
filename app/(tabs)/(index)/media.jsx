...
export default function Media() {
  ...
  const [linkSearchText, setLinkSearchText] = useState(null);
  ...
  useEffect(() => {
    if(!linkSearchText || !local.data) {
      return linkMatches.reset();
    }

    if (local.data.type === "movie") {
      linkMatches.fetchMovies(linkSearchText);
    } else {
      linkMatches.fetchShows(linkSearchText);
    }
    linkModalRef.current?.present();
  }, [linkSearchText, local.data])
  ...
  async function handleLink() {
    ...
    // If media doesn't have a link, show link modal
        ...
        setLinkSearchText(local.data.title);
        ...
        <Scroll gap={15}>
            ...
            <Backdrop backdrop_path={imagesNL.data?.backdrops[0]?.file_path}>
              ...
              >
                {local.data?.type === "movie" && (
                  <PlayButton mediaId={local.data?.media_id} />
                )}
              </View>
            </Backdrop>
          ...
          {local.data?.type === "show" && (
            <>
              ...
              />
              <EpisodesScroll
                loading={local.loading || episodes.loading}
                mediaId={local.data?.media_id}
                data={episodes.data}
